...
def interpretar_coef(nome, coef, pvalue):
    """Gera texto interpretativo para cada coeficiente."""
...
def montar_painel(cur):
    ...
    for estado in sorted(mvi.keys()):
        for ano in [2022, 2023, 2024]:
            ...
            # descarta linha se MVI ou policiamento ausentes
            ...
            registros.append({
                ...
                'ano':          ano,
                'mvi':          mvi_val,
                # converte para R$ bilhões para coeficientes interpretáveis
...
def main():
    print('=== Regressão Múltipla — Segurança Pública ===')
    ...
    for m in todos_modelos:
        ...
        ''', (
            ...
            m['n_obs'], int(m['significativo']),
        ))

    # ── Salva tabela de coeficientes
    cur.execute('''
    ...
    for c in todos_coefs:
        ...
        ''', (
            ...
            c['t_stat'], c['p_value'], int(c['significativo']), c['interpretacao'],
        ))

    conn.commit()
    cur.close()
    conn.close()
    print('\n✓ Concluído. Tabelas criadas: regressao_modelo, regressao_coeficientes.')
    print('  Instale statsmodels se necessário: pip install statsmodels')
